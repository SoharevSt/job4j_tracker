...
/*
В интерфейсе Map<K, V> определен особый метод, который позволяет вычислить значение в том
случае, если такого ключа в отображении нет или значение ассоциированное с ключом равно null.
Посмотрим как определен этот метод:
V computeIfAbsent(K key, Function<? super K,? extends V> mappingFunction) - если в отображении
заданный ключ key еще не связан со значением (или значение равно null),
будет выполнено вычисление значение в соответствии с использованием заданной функции
mappingFunction и это значение будет введено в отображение.
Рассмотрим небольшой пример:
Map<Integer, String> map = new HashMap<>();
map.put(1, "first"); // добавляем пару ключ-значение
map.computeIfAbsent(2, key -> "second" + "_" + key); // вычисляем значение для ключа,
которого нет в отображении
System.out.println(map.get(2)); // будет выведено second_2
map.computeIfAbsent(1, key -> "first" + "_" + key); // вычисляем значение для ключа,
который есть в отображении
    ...
    public static Map<Integer, String> collectData(Map<Integer, String> names, List<User> users) {
        for (User user : users) {
            ...
            if (names.containsKey(id)) {
                names.computeIfPresent(id, (key, value) -> value + user.getName());
            } else {
                names.computeIfAbsent(id, key -> user.getName());
            }
        }
        return names;
    }
