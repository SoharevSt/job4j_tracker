...
/*
В Java 8 в интерфейс Map<K, V> были добавлены особые методы, которые позволяют изменять
записи в отображении с помощь лямбда-выражений. Рассмотрим следующий метод:
V computeIfPresent(K key, BiFunction<? super K, ? super V,? extends V> remappingFunction) -
если отображение содержит значение для указанного ключа и не равно null,
выполняется вычисление нового сопоставления ключ - значение с учетом исходного ключа
и текущего сопоставленного значения.
...
map.computeIfPresent(1, (key, value) -> value + "@root"); // изменяем значение
которое ЕСТЬ в отображении
